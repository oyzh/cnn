...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        x, cache_x = word_embedding_forward(captions_in, W_embed) #(N,T,W)
        if self.cell_type == 'rnn':
            h, cache_rnn = rnn_forward(x, h0, Wx, Wh, b) #(N, T, H)
        else:
            h, cache_lstm = lstm_forward(x, h0, Wx, Wh, b)
        out, cache_affine = temporal_affine_forward(h, W_vocab, b_vocab) #(N,T,V)
    ...
    def sample(self, features, max_length=30):
        ...
        # a loop.                                                                 #
        ###########################################################################
        prev_h = features.dot(W_proj) + b_proj #(N, D) initilize hidden
        prev_c = np.zeros_like(prev_h)
        start = np.zeros((N), np.int32) + self._start #
        ...
        for i in xrange(max_length):
            ...
            affine_prev_h = prev_h.reshape((prev_h.shape[0],1,prev_h.shape[1]))
            out, _ = temporal_affine_forward(affine_prev_h, W_vocab, b_vocab)
            captions[:,i] = np.argmax(out, 2).reshape(-1)
